...
# moe: modulus of elasticity  ---> 用于截面组装
# nu: poisson's ratio         ---> 用于截面组装
# matTag: material used for fiber section
# secTag: the section tag for this section
# flayer: number of meshed layers of flange ---> 翼缘厚度方向的纤维数
# wlayer: number of meshed layers of web    ---> 腹板厚度方向的纤维数
# method: an enumerate: method='L': mesh by max length; method='N': mesh by number  ---> 一般选数量，决定纤维格子的尺寸也可以，最后还是换算成数量
# alphaf: max length or mesh number for flange    ---> 沿翼缘长度方向的纤维数
# alphaw: max length or mesh number for web       ---> 沿腹板长度方向的纤维数
